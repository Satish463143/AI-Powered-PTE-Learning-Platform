import React, { useState, useRef } from 'react';
import { FaPlay, FaPause } from 'react-icons/fa';
import Swal from 'sweetalert2';

const Question = ({ data, onAnswerChange }) => {
    const [selectedWords, setSelectedWords] = useState(new Set());
    const [isAudioPlaying, setIsAudioPlaying] = useState(false);
    const audioRef = useRef(null);

    const playAudio = () => {
        if (!isAudioPlaying && data?.question?.audioUrl) {
            audioRef.current = new Audio(data.question.audioUrl);
            audioRef.current.onended = () => {
                setIsAudioPlaying(false);
            };
            audioRef.current.play();
            setIsAudioPlaying(true);
        } else if (isAudioPlaying && audioRef.current) {
            audioRef.current.pause();
            setIsAudioPlaying(false);
        }
    };

    const handleWordClick = (wordIndex) => {
        const newSelectedWords = new Set(selectedWords);
        if (newSelectedWords.has(wordIndex)) {
            newSelectedWords.delete(wordIndex);
        } else {
            newSelectedWords.add(wordIndex);
        }
        setSelectedWords(newSelectedWords);
        onAnswerChange({
            selectedWords: Array.from(newSelectedWords),
            isComplete: newSelectedWords.size > 0
        });
    };

    const renderText = () => {
        if (!data?.question?.text) return null;

        const words = data.question.text.split(' ');
        return words.map((word, index) => (
            <span
                key={index}
                onClick={() => handleWordClick(index)}
                className={`cursor-pointer mx-1 py-1 px-2 rounded ${
                    selectedWords.has(index) ? 'bg-yellow-200' : ''
                } hover:bg-gray-100`}
            >
                {word}
            </span>
        ));
    };

    return (
        <div className="listening-question">
            <h2 className="font-bold mb-4">Highlight Incorrect Words</h2>
            
            <div className="audio-container p-6 bg-white rounded-lg shadow-sm mb-6">
                <button
                    onClick={playAudio}
                    className="audio-btn"
                    style={{
                        padding: '12px 24px',
                        background: '#2daef5',
                        color: 'white',
                        border: 'none',
                        borderRadius: '5px',
                        cursor: 'pointer',
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px'
                    }}
                >
                    {isAudioPlaying ? <FaPause /> : <FaPlay />}
                    {isAudioPlaying ? 'Pause Audio' : 'Play Audio'}
                </button>
            </div>

            <div className="text-container p-6 bg-white rounded-lg shadow-sm">
                <div className="instructions mb-4 text-gray-600">
                    Click on the words that are different from what you hear in the audio.
                </div>
                <div className="text-content text-lg leading-relaxed">
                    {renderText()}
                </div>
            </div>
        </div>
    );
};

const HighlightIncorrectWords = ({ data, onSubmit, isLoading }) => {
    const [answer, setAnswer] = useState({ selectedWords: [], isComplete: false });

    const handleAnswerChange = (newAnswer) => {
        setAnswer(newAnswer);
    };

    const handleSubmit = async () => {
        if (!answer.isComplete) {
            await Swal.fire({
                title: 'No Words Selected',
                text: 'Please highlight at least one incorrect word before submitting.',
                icon: 'warning'
            });
            return;
        }

        await Swal.fire({
            title: 'Submit Answer?',
            text: 'Are you sure you want to submit your answer?',
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Yes',
            cancelButtonText: 'No'
        }).then((result) => {
            if (result.isConfirmed) {
                onSubmit(answer);
            }
        });
    };

    if (isLoading) {
        return <div>Loading...</div>;
    }

    return (
        <>
            <div className="listening-wrapper container">
                <div className="mock-question-container">
                    <div className="question">
                        <Question
                            data={data}
                            onAnswerChange={handleAnswerChange}
                        />
                    </div>
                </div>
            </div>
            <div className='submit_button'>
                <button
                    onClick={handleSubmit}
                    className='primary-btn'
                    disabled={!answer.isComplete || isLoading}
                >
                    Submit
                </button>
            </div>
        </>
    );
};

export default HighlightIncorrectWords; 