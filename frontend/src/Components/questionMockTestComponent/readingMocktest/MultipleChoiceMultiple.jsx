...
const Question = ({ data, onAnswerChange }) => {
    ...
    const handleAnswerSelect = (answerId) => {
        ...
        onAnswerChange({
            ...
            isComplete: newSelectedAnswers.length > 0
        });
    };

    return (
        <div className="reading-question">
            <h2 className="font-bold mb-4">Multiple Choice (Multiple Answers)</h2>
...
const MultipleChoiceMultiple = ({ data, onSubmit, isLoading }) => {
    const [answer, setAnswer] = useState({ selectedAnswers: [], isComplete: false });
    ...
    const handleSubmit = async () => {
        if (!answer.isComplete) {
            await Swal.fire({
                ...
                text: 'Please select at least one answer before submitting.',
                icon: 'warning'
            });
            return;
        }
