...
const shortAnswerAudios = import.meta.glob('../../../assets/audio/Speaking/answer_short_question/*.mp3', { eager: true });

// Helper function to get random audio
const getRandomAudio = () => {
  const audios = Object.values(shortAnswerAudios).map(module => module.default);
  const randomIndex = Math.floor(Math.random() * audios.length);
  return audios[randomIndex];
};

const RecordingBox = ({ recordingTime, status }) => {
    ...
    return (
      <div className="recording_aboutMe">
        ...
        <h3 className={`status-text ${status === "Playing Audio" ? 'audio-playing' : 'recording'}`}>
          {status}
        </h3>
        <div className="progress-bar-container">
          <div 
            className={`progress-bar-fill ${status === "Playing Audio" ? 'audio-playing' : 'recording'}`}
            style={{ width: calculateProgress() }}
          ></div>
        </div>
        {recordingTime > 0 && status !== "Playing Audio" && (
...
const AudioPlayingBox = ({ audioPlaying, prepTime }) => {
    ...
    const calculateProgress = () => {
        if (prepTime > 0) {
            return `${((4 - prepTime) / 4) * 100}%`;
        } else if (audioPlaying) {
            return `${audioPlaying.progress * 100}%`;
        }
        return '100%';
    };
...
const Question = ({ data, prepTime, audioPlaying, recordingTime, status, audioRef }) => (
    <div>
        ...
        <AudioBox audioUrl={getRandomAudio()} audioRef={audioRef} />
...
const ShortAnswer = ({ onNext, updateProgress }) => {
    ...
    const handleNextQuestion = async () => {
        try {
            ...
            if (questionIndex < totalQuestions - 1) {
                ...
                // Generate writing questions before moving to writing section
                try {
                    ...
                    const response = await startTest({
                        ...
                        section: 'writing' // Add section parameter to only generate writing questions
                    }).unwrap();

                    if (response?.result?.questions) {
                        const writingQuestions = response.result.questions.filter(q => 
                            q.questionType.includes('summarizeWrittenText') || 
                            q.questionType.includes('writeEssay')
                        );
                        
                        if (writingQuestions.length > 0) {
                            // Move to writing section with generated questions
                            onNext(writingQuestions);
                        } else {
                            throw new Error('No writing questions generated');
                        }
                    } else {
                    ...
                    setIsGeneratingQuestions(false); // Make sure to reset loading state on error
                    Swal.fire({
                        title: 'Error',
    ...
    if (isGeneratingQuestions) {
        return (
            <div className="loading-container" style={{ 
                ...
                <h2>Generating writing questions...</h2>
                <p>Please wait</p>
            </div>
        );
    }
