...
const Question = ({ data, prepTime, audioPlaying, recordingTime, status, audioRef, isLoading }) => (
    <div>
      ...
      {data?.audioUrl && (
        <AudioBox audioUrl={data.audioUrl} audioRef={audioRef} />
      )}
...
const RetellLecture = ({ onNext, updateProgress, questions: initialQuestions, isGeneratingQuestions }) => {
    ...
    const handleNextQuestion = async () => {
        try {
            ...
            setIsLoading(true);
            // Save the current answer if we have recorded audio
            if (audioChunksRef.current.length > 0) {
                const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/wav' });
                const formData = new FormData();
                formData.append('file', audioBlob, 'recording.wav');
                formData.append('actionType', 'next');
                ...
                formData.append('userAnswer', JSON.stringify({
                    recordingTime: 40 - recordingTime
                }));
                formData.append('mockTestId', questions[0]?.mockTestId);
                
                await startTest(formData).unwrap();
            }

            if (questionIndex >= totalQuestions - 1) {
                // Call onNext first to trigger the loading state in parent component
                onNext();
            } else {
    ...
    if (isGeneratingQuestions || !questions.length) {
        return (
            <div className="loading-container" style={{ 
                ...
                <h2>Loading Retell Lecture questions...</h2>
                <p>Please wait</p>
            </div>
        );
    }
