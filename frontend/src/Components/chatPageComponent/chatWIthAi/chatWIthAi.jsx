...
const chatWithAi = ({ isSoundOn, selectedScore, selectedContent,selectedSection, resetChat, setShowChatWithAi, showChatWithAi, setMessages }) => {
  ...
  useEffect(() => {
    if (selectedContentFromHomePage && !hasResponded && !processedFromHomePage.current) {
      ...
      const userMessage = `Selected option from home page: ${selectedContentFromHomePage}`;
  ...
  useEffect(() => {
    ...
    if (selectedScore && selectedContent && !hasResponded && 
      ...
      setShowChatWithAi(false);
      const userMessage = `Selected score: ${selectedScore}, Question Type: ${selectedContent}, Section: ${selectedSection}`;
      setMessages(prev => [...prev, { type: 'user', text: userMessage }]);
  ...
  const handleOptionClick = (option) => {
    ...
    const sendMessage = async () => {
      try {
        ...
        const response = await getChat(chatData);
        setMessages(prev => [...prev, { type: 'ai', text: response?.data?.result }]);
